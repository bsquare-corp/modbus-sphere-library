...
bool ValidateCRC( uint8_t *message, int inputLength ) {
    if (inputLength > CRC_FOOTER_LENGTH) {
        uint16_t crcValCalculated = GetCRC(message, inputLength - CRC_FOOTER_LENGTH);
        uint16_t crcValRead = (uint16_t)((message[inputLength - (CRC_FOOTER_LENGTH - 1)]<<8) | (message[inputLength - CRC_FOOTER_LENGTH]));
        if (crcValCalculated == crcValRead) {
            return true;
        }
    }
    return false;
...
bool AddCRC( uint8_t *message, int inputLength, int maxInputLength) {
    if (inputLength + CRC_FOOTER_LENGTH <= maxInputLength) {
